package com.example.handigab_porteur.security;
...
public class CipherStreamSystem {
	...
	public void shareKeys(Socket socket) {
		try {
			...
			Log.d(TAG, "Envoi d'une clé de " + myPublicKeyBytes.length);
			out.writeInt(myPublicKeyBytes.length);
			out.flush();

			Log.d(TAG, "Clé envoyé : " + myPublicKeyBytes);
			...
			Log.d(TAG, "Lecture d'une clé de " + keySize + " bits");

			itsPublicKeyBytes = new byte[keySize];
			in.readFully(itsPublicKeyBytes, 0, keySize);
			Log.d(TAG, "Clé reçu : " + itsPublicKeyBytes);
			...
			Log.d(TAG, "Partage des clés terminé.");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @throws Exception
	 */
	public void shareKeys(BluetoothSocket socket) {
		try {
			DataOutputStream out = new DataOutputStream(socket.getOutputStream());
			DataInputStream in = new DataInputStream(socket.getInputStream());
